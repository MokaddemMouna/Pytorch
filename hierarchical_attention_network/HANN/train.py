...
from HANN.model import HierarchialAttentionNetwork
from HANN.utils import *
from HANN.datasets import HANDataset
...
def main():
    ...
    # DataLoaders
    train_dataset = HANDataset(data_folder, 'train')
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (documents, sentences_per_document, words_per_sentence, labels) in enumerate(train_loader):
        ...
        # Loss, the loss is averaged across observations for each minibatch
        ...
        nb_samples = labels.size(0)
        accuracy = correct_predictions / nb_samples

        # Keep track of metrics

        losses.update(loss.item(), nb_samples)
        batch_time.update(time.time() - start)
        accs.update(accuracy, nb_samples)
